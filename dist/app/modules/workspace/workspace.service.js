...
import { ActivityAction, ActivityEntity, UserStatus, WorkspaceRole, } from "../../../generated/prisma/enums";
const createWorkspace = async (payload, userId) => {
    ...
    if (existingWorkspace) {
        throw new AppError(status.CONFLICT, "You already have a workspace with this name.");
    }
...
const updateWorkspace = async (workspaceId, userId, payload) => {
    ...
    if (membership.role !== WorkspaceRole.OWNER &&
        ...
        throw new AppError(status.FORBIDDEN, "You do not have permission to update this workspace.");
    }
...
const deleteWorkspace = async (workspaceId, userId) => {
    ...
    if (membership.role !== WorkspaceRole.OWNER) {
        throw new AppError(status.FORBIDDEN, "Only the workspace owner can delete the workspace.");
    }
    const workspace = await prisma.workspace.findFirst({
        where: {
            id: workspaceId,
...
const addMember = async (workspaceId, requesterId, payload) => {
    ...
    if (requester.role !== WorkspaceRole.OWNER &&
        ...
        throw new AppError(status.FORBIDDEN, "You do not have permission to add members");
    }
    const workspace = await prisma.workspace.findFirst({
        where: {
            id: workspaceId,
    ...
    if (existingMember) {
        throw new AppError(status.CONFLICT, "User is already a member of this workspace");
    }
    if (requester.role === WorkspaceRole.ADMIN && role === WorkspaceRole.ADMIN) {
        throw new AppError(status.FORBIDDEN, "Only the workspace owner can assign the ADMIN role.");
    }
    const result = await prisma.$transaction(async (tx) => {
...
const getWorkspaceMember = async (workspaceId, requesterId, memberId) => {
    const requester = await prisma.workspaceMember.findUnique({
        where: {
            workspaceId_userId: {
                workspaceId,
...
const updateMemberRole = async (workspaceId, requesterId, memberId, payload) => {
    const requester = await prisma.workspaceMember.findUnique({
        where: {
            workspaceId_userId: {
                workspaceId,
    ...
    if (requester.role !== WorkspaceRole.OWNER) {
        throw new AppError(status.FORBIDDEN, "Only the workspace owner can change member roles.");
    }
    const member = await prisma.workspaceMember.findUnique({
        where: {
            workspaceId_userId: {
    ...
    if (member.role === WorkspaceRole.OWNER) {
        throw new AppError(status.FORBIDDEN, "The workspace owner's role cannot be changed.");
    }
    const result = await prisma.$transaction(async (tx) => {
...
const removeMember = async (workspaceId, requesterId, memberId) => {
    const requester = await prisma.workspaceMember.findUnique({
        where: {
            workspaceId_userId: {
                workspaceId,
    ...
    if (requester.role !== WorkspaceRole.OWNER &&
        requester.role !== WorkspaceRole.ADMIN) {
        throw new AppError(status.FORBIDDEN, "You do not have permission to remove members.");
    }
    const member = await prisma.workspaceMember.findUnique({
        where: {
            workspaceId_userId: {
    ...
    if (member.role === WorkspaceRole.OWNER) {
        throw new AppError(status.FORBIDDEN, "The workspace owner cannot be removed.");
    }
    if (requester.role === WorkspaceRole.ADMIN &&
        member.role === WorkspaceRole.ADMIN) {
        throw new AppError(status.FORBIDDEN, "An admin cannot remove another admin.");
    }
    await prisma.$transaction(async (tx) => {
