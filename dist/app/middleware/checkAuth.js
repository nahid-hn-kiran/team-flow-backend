...
export const checkAuth = (...authRoles) => async (req, res, next) => {
    try {
        ...
        if (sessionToken) {
            ...
            if (sessionExists && sessionExists.user) {
                ...
                if (authRoles.length > 0 && !authRoles.includes(user.role)) {
                    throw new AppError(status.FORBIDDEN, "Forbidden access! You do not have permission to access this resoure.");
                }
            }
        }
        const token = cookieUtils.getCookie(req, "accessToken");
        const verifyToken = jwtUtils.verifyToken(token, envVars.ACCESS_TOKEN_SECRET);
        ...
        if (authRoles.length > 0 &&
            !authRoles.includes(verifyToken.data.role)) {
            throw new AppError(status.FORBIDDEN, "Forbidden access! You do not have permission to access this resoure.");
        }
        req.user = {
